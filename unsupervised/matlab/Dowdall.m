...
for times = 1:num_runs
    ...
    dowdall_score = 1 ./ rank;
    dowdall_score = sum(dowdall_score,1);
    
    [~,res] = sort(-dowdall_score,3);
    [~,res] = sort(res,3);

    res = reshape(res,querynum,item_num);
    
    % save the .mat file which consist of the rank result
    save('D:\Code of RA\Preflib\results\ice-cream\rank-based\rank-result-ice-cream-Dowdall.mat', 'res');
